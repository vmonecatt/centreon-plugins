...
package apps::proxmox::backupserver::restapi::custom::api;
...
sub check_options {
    ...
    $self->{port} = (defined($self->{option_results}->{port})) ? $self->{option_results}->{port} : 8007;
...
sub settings {
    ...
    if (defined($self->{ticket})) {
        $self->{http}->add_header(key => 'Cookie', value => 'PBSAuthCookie=' . $self->{ticket});
    }
...
Set Proxmox Backup Server Realm (C<pam>, C<pbs>, C<openid>, C<ldap> or C<ad>) (default: C<pam>).
